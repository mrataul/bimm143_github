...
There are many graphics systems in R for making plots and figures. These include so-called *"base R" graphics* like `plot()` and **ggplot2** which is an add on package. 
...
To install it, we can use the function `install.packages()`, however we never run this function in the document because it would redownload it constantly. Instead this is done in the console.
...
Let's add a trendline that shows the relationship between speed + distance. 
